...
def block_conformal_margin(scores, alpha, block_count, block_quantile=0.9, rng=None, tie_jitter=0.0):
    scores = np.asarray(scores, dtype=np.float64)
    scores = scores[np.isfinite(scores)]
    if scores.size == 0:
        return 0.0
...
def validate_coverage(y_true, y_lower, y_upper, target_coverage):
    """Checks if the actual coverage matches the target coverage within tolerance."""
    if y_true.size == 0:
        return None
    coverage = np.mean((y_true >= y_lower) & (y_true <= y_upper))
    return coverage

def fit_mondrian_bins(features, scores, alpha, edges_list, min_bin, shrinkage, global_c, rng=None, tie_jitter=0.0):
    ...
    # Check for empty bins or low samples
    small_bins = np.sum(counts < min_bin)
    if small_bins > 0:
        # We could warn here, but for now we just rely on shrinkage/fallback
        pass
...
def predict_quantile_ensemble(x_train, y_train, x_val, y_val, x_calib, x_test, quantile, args, device, seed_base):
...
def predict_sigma_quantile_ensemble(x_train, y_train, x_val, y_val, x_pred, args, device, seed_base):
...
def predict_sigma_mlp(x_train, y_train, x_val, y_val, x_pred, args, device, seed_base):
